...
public class DeveloperService : IDeveloperService
{
    ...
    public async Task<DeveloperResponseDto> CreateAsync(CreateDeveloperDto dto)
    {
        if (!CedulaValidator.ValidarCedula(dto.Cedula))
            throw new BusinessValidationException("La cédula ingresada no es válida según el algoritmo de verificación ecuatoriano.");

        if (await _developerRepository.ExistsByCedulaAsync(dto.Cedula))
            throw new BusinessValidationException("La cédula ya está registrada.");
